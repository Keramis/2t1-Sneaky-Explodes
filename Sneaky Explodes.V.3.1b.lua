...
local checkJL = menu.add_feature("Check for player joining/leaving", "toggle", sneakyM.id, function(feat)
    if feat.on then
        EventJoin()
        EventLeave()
        system.wait(1000)
...
menu.add_feature("Force recheck playertable", "action", sneakyM.id, function()
    PlayerTable = {}
    PlayerArray()
...
--force update button
menu.add_player_feature("////" .. "Force Refresh Players" .. "////", "action", sneakyAB.id, function()
    Features["Explode one"].feat:set_str_data(PlayerTable)
end)
...
--force update button
menu.add_player_feature("////" .. "Force Refresh Players" .. "////", "action", sneakyAL.id, function()
    Features["Explode one loop"].feat:set_str_data(PlayerTable)
end)
