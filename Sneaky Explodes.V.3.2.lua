...
--[[
    Websites used for color encoding:
...
function MenuNotify(message, title, seconds, color)
    menu.notify(message, title, seconds, color)
end

SneakyExpldesName = "Sneaky Explodes V.3.2"

--notifies if the script is running
MenuNotify(SneakyExpldesName .. " is running.", SneakyExpldesName, 3, 0xFFF476CA)
MenuNotify("Made by Keramis/Scoopp", SneakyExpldesName, 3, 0xFFF476CA)
...
print("-----------------------")
print(SneakyExpldesName .. " -- Keramis / Scoopp")
...
event.add_event_listener("player_join", function()
    ...
    MenuNotify("Player joining, updating player table.", SneakyExpldesName, 1, 0xFF4CF0C2)
end)
...
event.add_event_listener("player_leave", function()
    ...
    MenuNotify("Player leaving, updating player table.", SneakyExpldesName, 1, 0xFF4CF0C2)
end)
...
function ReCheck()
    PlayerTable = {}
    PlayerArray()
    MenuNotify("Re-checked player table!", SneakyExpldesName, 1, 0xFFF0DB4C)
end
...
--script menu check for modders parent
local sneakyMM = menu.add_feature("Check lobby for modders", "parent", sneakyM.id)
...
PlayerFeature("Check god", "action", sneakyO.id, function(feat, pid)
    ...
    if godcheck == true and intCheck == 0 then
        MenuNotify("Player godmode is TRUE", "Godmode Check", 3, 221)
        player.set_player_as_modder(pid, SneakyE_godmode_failure)
    else
        MenuNotify("Player godmode is FALSE", "Godmode Check", 3, 221)
    end
    local ped1god = tostring(ped1)
    print(ped1god)
end)

Feature("Lobby Godmode Check", "action", sneakyMM.id, function(feat, pid)
    Godnumber = 0
    for i = 0, 31, 1 do
...
local SneakyE_health_failure = player.add_modder_flag("HEALTHCHECK")

Feature("Lobby Health Check", "action", sneakyMM.id, function(feat, pid)
    Healthnumber = 0
    for i = 0, 31, 1 do
        local playerH = player.get_player_health(i)
        local playerMH = player.get_player_max_health(i)
        local playerstring = tostring(player.get_player_name(i))
        if playerH > playerMH then
            player.set_player_as_modder(i, SneakyE_health_failure)
            MapNotify(playerstring .. "~r~" .. "~h~" .. " has modded health!", "Sneaky Explosions", 0)
        elseif playerH > 328 then
            player.set_player_as_modder(i, SneakyE_health_failure)
            MapNotify(playerstring .. "~r~" .. "~h~" .. " has modded health!", "Sneaky Explosions", 0)
        elseif playerMH > 328 then
            player.set_player_as_modder(i, SneakyE_health_failure)
            MapNotify(playerstring .. "~r~" .. "~h~" .. " has modded health!", "Sneaky Explosions", 0)
        end
    end
...
PlayerFeature("Sneaky ALL Water Hydrant BLAME", "action", sneakyAB.id, function(feat, pid)
    local blamer = player.get_player_ped(pid)
    local offset = v3(0.0, 0.0, -2)
    for i = 0, 31, 1 do
        local playerPOS = player.get_player_coords(i)
        Explode(playerPOS + offset, 13, false, DoWeHide, 0, blamer)
    end
end)
...
Features["Explode one"] = {feat = menu.add_player_feature("Blame for exploding: ", "action_value_str", sneakyAB.id, function(feat, pid)
    ...
    if theKilledI ~= 0 then
        MenuNotify("Player is in an interior, stopping explode.", SneakyExpldesName)
    elseif theKilledG == true then 
        MenuNotify("Player is in godmode, stopping explode.", SneakyExpldesName)
    else
...
PlayerFeature("////" .. "Force Refresh Players" .. "////", "action", sneakyAB.id, function()
    Features["Explode one"].feat:set_str_data(PlayerTable)
    MenuNotify("Refreshed players!", SneakyExpldesName, 1, 0xFFF0DB4C)
end)
...
PlayerFeature("Sneaky Water Hydrant (blames them)", "action", sneakyA.id, function(feat, pid)
    ...
    local playerPED = player.get_player_ped(pid)
    local offset = v3(0.0, 0.0, -2)
    Explode(playerPOS + offset, 13, false, DoWeHide, 0, playerPED)
end)
